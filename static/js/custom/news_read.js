...
$("#page_news_read").on('click', '#delete', function(){

    $.get('/modules/news_delete/?id='+$(this).attr('data-id'), function (){});
    
    $.mobile.changePage("/news_category/?category="+$(this).attr('data-category'), {
        transition: "flip", 
        reloadPage: true
    });
...
$('#page_news_read').bind('pageshow', function () {
...
$('#page_news_read').bind('pagehide', function () {
    $('#page_news_read').remove();
});

$("#page_news_read #next").on('click', next_page);
$("#page_news_read #back").on('click', back_page);

$('#page_news_read').on('swipeleft','#comments', function(){ 
...
$('#page_news_read').on('swiperight','#comments', function(){ 
...
$("#page_news_read").on('click', '#submit', function(){

    $('#comments_send').addClass("ui-disabled").trigger('updatelayout');

    $.ajax({
        type: "POST",
        url: "/modules/comments_add/?id="+$('#comments').attr('data-news')+"&type=news",
        cache: false,
        dataType: 'json',
...
$("#page_news_read").on('taphold', '#comments li', function(){

    if($('#page_news_read').attr('data-admin') == 0 || altKey)
        return;

    comment = $(this).attr('data-id');
    $('#page_news_read #popup_comment').popup("open", {
        positionTo: "window", 
        transition: "flip"
    });
...
$('#comments_list li').live('click', function (){
    ...
    if($('#page_news_read').attr('data-auth') == 0)
        return;
...
$("#page_news_read").on('click', '#delete_comment', function(){
...
function load_comments(n, status){
    ...
    $.getJSON('/modules/comments_get/?id='+$('#comments').attr('data-news')+'&n='+n+'&type=news', function(data) {
